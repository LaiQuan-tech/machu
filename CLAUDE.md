...
- **Hero 神尊在平板直立會被切（`HERO_DEITIES`）**：桌機那組尺寸原本是純 vh，平板直立（iPad 第十代 820×1180）螢幕又高又窄，整組寬到 949px 卻只有 820px 可放，最左邊的濟公被切掉快一半。解法是每個 vh 都包 `min(A vh, A×0.79 vw)`，**0.79 這個比例八個值要一起改**；長寬比 1/0.79≈1.266 是分界，比這寬走 vh（桌機完全不受影響），比這窄走 vw。同時要把 `sm:max-w-[…vw]` 改成 `sm:max-w-none`——那三個上限加起來是 156vw，窄螢幕上同時觸頂反而是撐寬的元凶。負邊距要寫 `mb-[calc(min(…)*-1)]`，寫 `-mb-[min(…)]` 會產出無效的 `-calc(...)`。改完在 820×1180／1024×1366／1280×800 量過。驗收標準是**三尊彼此不蓋到頭、且用實際像素判定**（頭部矩形會誤判，四角本來就透明），現況三個尺寸都 ≤0.01%。
- **Hero 三尊的身分弄錯過兩次（`HERO_DEITIES`）**：左＝濟公、中＝**天上聖母三媽（主神、黃袍金冠）**、右＝**天上聖母二媽（橘袍黑面）**。
  2026-09-01 的「正名」把兩尊媽祖對調了——橘袍那尊從專案第一天就叫 `hero-erma`（`scripts/build-hero-assets.js` 的 JOBS 註解「二媽 拍在黃牆前」是獨立佐證），卻被改名成三媽搬到中間，等於把主神標成別人，而 `alt` 會被螢幕閱讀器念出來。2026-09-02 換回來。
  **只看檔名不看圖就會再錯一次**：要分辨兩尊媽祖，取袍身下半部的色相中位數——三媽約 45°（黃）、二媽約 21°（橘）。濟公約 20°，跟二媽接近，這招只用來分辨兩尊媽祖。
  換照片時 `drop` 一定要重算：`臉高 = 圖高×(1−臉在圖中的相對位置) − drop`，兩張媽祖的取景差了 8 個百分點，沿用舊值臉的高低順序就會翻掉。臉的位置用「每 5% 一條網格」目視讀，別寫顏色規則自動找——二媽是黑面，她的手與軀幹跟臉同色，程式會抓到胸口。
  `scripts/build-hero-assets.js` **產不出現在 public/ 那三張**：2026-09-01 起廟方直接給已去背的原圖，當時只裁 alpha 邊界＋縮到 1600 高（腳本輸出的是 1400）。要重跑先確認手上的原始檔是不是未去背。
...
- 2026-09-02 Hero 兩尊媽祖身分換回（三媽黃袍居中為主神、二媽橘袍在右），drop／交疊量重算
