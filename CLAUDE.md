...
- **祭祀行事曆（`/calendar`，2026-09-02 建置、2026-09-09 上線）**：神明聖誕與壇務活動的年度條列。
  ...
  **同一位神明的多尊合併成一筆**：天上聖母（七尊）、濟公師父（兩尊）、三太子（三尊）都同日，一尊一筆會讓前台出現連續七列日期一樣只有名字不同的項目。合併後 `deity_id` 設 NULL（一對多，schema 是 1:1 接不住；硬指其中一尊會讓其餘憑空消失，比留白更誤導），尊號寫進 `note`。同日的土地公是另一位神明，不併。
