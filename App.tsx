 ...
 *
 * ── 身分（**這裡弄錯過兩次，改之前先讀完**）──
 *   左＝濟公活佛、中＝天上聖母三媽、右＝天上聖母二媽。
 *   **三媽是本壇主神，黃袍金冠；二媽是橘袍黑面。**
 *   檔名 hero-jigong／hero-sanma／hero-erma 與 alt 都照這個對應，
 *   index.html 的 preload 指向 hero-sanma（主神）。
 *
 *   想快速確認誰是誰：取袍身下半部的色相中位數，
 *   **三媽約 45°（黃）、二媽約 21°（橘）**。濟公約 20°，跟二媽接近，
 *   所以這招只用來分辨兩尊媽祖，不要拿來跟濟公比。
 *
 *   為什麼要寫這麼白：2026-09-01 那次「正名」把兩尊媽祖對調了——
 *   橘袍那尊從專案第一天就叫 hero-erma（scripts/build-hero-assets.js 的
 *   JOBS 註解「二媽 拍在黃牆前」是獨立佐證），卻被改名成三媽搬到中間，
 *   等於把主神標成別人，而 alt 會被螢幕閱讀器念出來。2026-09-02 換回來。
 *   **只看檔名不看圖，就會再錯一次。**
 *
 * ── 構圖：主神居中、三尊高低錯落（廟方要求）──
 *   三媽最高最大、疊在最上層（z-3），臉的高度也最高。
 *   臉高順序是 三媽 > 濟公 > 二媽（頂端順序不同，因為冠帽高度差很多，
 *   別拿頂端當驗收標準）。
 *
 *   drop 是每尊往畫面下方沉多少，決定臉的高度：
 *     臉在畫面上的高度 = 圖高 ×（1 − 臉在圖中的相對位置）− drop
 *   **換照片就要重量「臉在圖中的相對位置」再解一次 drop**，
 *   沿用舊值必定跑掉——同樣是媽祖，兩張的取景差了 8 個百分點。
 *   實測值（臉中心佔圖高的比例）：濟公 0.23、三媽 0.36、二媽 0.28。
 *   量法：把圖畫成每 5% 一條的網格再目視讀刻度。用顏色規則自動找會失敗，
 *   二媽是黑面，她的手與軀幹跟臉同色，程式會抓到胸口去。
 *
 * ── 驗收依據是「彼此不蓋到頭」，而且要用實際像素判定 ──
 *   每尊的頭部（含冠帽）由 alpha 輪廓剖面算出：由上往下找寬度首次超過
 *   最大寬 55% 的那一列當肩線，肩線以上就是頭。實測：
 *     濟公 y 0–32.9%、x 22.5–76.5%；三媽 y 0–41.9%、x 33.0–88.5%；
 *     二媽 y 0–34.4%、x 24.0–86.5%。
 *   判定要取「上層那尊的不透明像素 ∩ 下層那尊頭部的不透明像素」。
 *   只用頭部矩形會誤判——矩形四角本來就是透明的。
 *   2026-09-02 實測：桌機 1280×800 與平板 820×1180 都是 0.00/0.00/0.01%，
 *   手機 375×812 是 0.00/0.00/0.00%。
 *   手機那組的交疊量是這樣定出來的：二媽的頭在自己圖框裡偏左（左緣 24%），
 *   沿用舊的 -ml-[16vw] 會壓掉她 11% 的冠帽，放鬆到 11vw 才歸零。
 *
 * ── 單位規則 ──
 ...
 *   目前手機總寬約 113vw，左右各裁掉 6.4vw（裁到的是濟公的左袖與
 *   二媽的右袍襬，不是臉）。
 *
 ...
 *
 * ── 其他 ──
 *   layer：矮的在前、高的在後，像神桌的排法。但主神例外，她疊最上層。
...
const HERO_DEITIES: Array<{ src: string; fallback: string; name: string; size: string; drop: string; gap?: string; layer: string; priority?: boolean }> = [
  ...
  // 中：天上聖母三媽（黃袍金冠），本壇主神。最高最大、疊在最上層，優先權與 preload 都給她
  //（index.html 的 preload 要指向同一張，兩邊不一致等於預載了不是主角的那張）
  { src: '/hero-sanma.webp',  fallback: '/hero-sanma.png',  name: '天上聖母三媽',
    size: 'h-[108vw] max-w-[88vw] sm:h-[min(98vh,77.42vw)] sm:max-w-none',
    drop: '-mb-[10.5vw] sm:mb-[calc(min(9.7vh,7.66vw)*-1)]',
    gap: '-ml-[15vw] sm:ml-[calc(min(10.5vh,8.29vw)*-1)]', layer: 'z-[3]', priority: true },
  // 右：天上聖母二媽（橘袍黑面）。刻意與濟公不同高，做出「高低錯落」而不是對稱的兩根柱子
  { src: '/hero-erma.webp',   fallback: '/hero-erma.png',   name: '天上聖母二媽',
    size: 'h-[62vw] max-w-[76vw] sm:h-[min(74vh,58.46vw)] sm:max-w-none',
    drop: '-mb-[11.5vw] sm:mb-[calc(min(11vh,8.69vw)*-1)]',
    gap: '-ml-[11vw] sm:ml-[calc(min(15.5vh,12.25vw)*-1)]', layer: 'z-[1]' },
];
